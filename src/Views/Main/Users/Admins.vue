...
export default {
	...
	computed: {
		...
		...mapGetters("Users", ["getdata", "getUsersData"]),

	},
	methods: {
		...mapActions("Roles", ["GetAdminPermissions", "UpdateUserPermission"]),
		...mapActions("Users", ["GetAdminUsers", "CreateAdmin", "UpdateAdmin", "DeleteAdmin"]),
		...
		createFunc() {
			...
			if (this.checkAddValidation()) {
				...
				this.CreateAdmin(this.data).then(Response => {
					...
					loading.close();
					this.GetAdminUsers(this.dataSearch);
		...
		UpdateFunc() {
			if (this.checkUpdateValidation()) {
				...
				this.UpdateAdmin(this.data).then(Response => {
		...
		checkUpdateValidation() {
			if (this.data.id == "") {
				this.$moshaToast('خطأ في تحديد المستخدم', {
					hideProgressBar: 'false',
					position: 'top-center',
					showIcon: 'true',
					swipeClose: 'true',
...
<template>
	...
	<div class="page-header">
		<div class="row">
			<div class="col-sm-12">
				<h3 class="page-title">إدارة مدراء النظام</h3>
				<!-- <ul class="breadcrumb">
				...
				<div class="card-body">
					<div class="table-responsive">
						<table class="datatable table table-hover table-center mb-0">
							...
							<tbody>
								<tr v-for="(item, index) in getUsersData">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div>
						...
						<button v-on:click="UpdateFunc()" class="btn btn-primary btn-block">إضافة</button>
					</div>
				</div>
			</div>
		</div>
