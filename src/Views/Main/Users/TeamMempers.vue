...
import { UserStatusEnum, UserTypeEnum } from '@/config/config.js'
...
export default {
	...
	computed: {
		...
		...mapGetters("Users", ["getdata", "getTeamMembersData", "getTeamsData", "getUserInfo"]),
	...
	methods: {
		...mapActions("Roles", ["GetPermissions", "UpdateUserPermission"]),
		...mapActions("Users", ["GetTeamMembers", "CreateTeamMember", "UpdateTeamMember", "DeleteTeamMember", "GetTeams"]),
		...
		initFunc() {
			...
			this.GetTeams({ name: "" });
			this.GetPermissions();
			this.GetTeamMembers(this.dataSearch).then(Response => {
				loading.close();
				console.log("data : ", this.getTeamMembersData);
			}).catch(error => {
		...
		createFunc() {
			if (this.checkAddValidation()) {
				// Include permissions in the create data
				this.data.permissions = this.checkedPermissions;
				...
				this.CreateTeamMember(this.data).then(Response => {
					...
					loading.close();
					this.GetTeamMembers(this.dataSearch);
		...
		checkAddValidation() {
			if (this.data.name == "") {
				...
				this.$moshaToast('تأكد من مطابقة كلمة المرور مع تأكيد كلمة المرور', {
					hideProgressBar: 'false',
					position: 'top-center',
					showIcon: 'true',
					swipeClose: 'true',
		...
		UpdateFunc() {
			if (this.checkUpdateValidation()) {
				// Include permissions in the update data
				this.data.permissions = this.updateCheckedPermissions;
				...
				this.UpdateTeamMember(this.data).then(Response => {
		...
		checkUpdateValidation() {
			if (this.data.id == 0) {
				this.$moshaToast('خطأ في تحديد عضو الفريق', {
					hideProgressBar: 'false',
					position: 'top-center',
					showIcon: 'true',
					swipeClose: 'true',
		...
		DeleteFunc() {
			...
			if (this.checkDeleteValidation()) {
				...
				this.DeleteTeamMember(this.data.id).then(Response => {
					console.log(Response);
		...
		async selectItem(id) {
			this.clearData();
			const foundItem = this.getTeamMembersData.find(element => element.id === id);
			...
			if (foundItem) {
				...
				this.data.moreInfo = foundItem.moreInfo || "";
				// Load user permissions
				if (foundItem.permissions && Array.isArray(foundItem.permissions)) {
					this.updateCheckedPermissions = foundItem.permissions;
				} else {
					this.updateCheckedPermissions = [];
				}
				// Update indeterminate states for all modules
		...
		clearData() {
			...
			this.checkModulePerAll = {};
			this.isIndeterminateStates = {};
			this.updateCheckedPermissions = [];
			this.updateCheckModulePerAll = {};
...
<template>
	...
	<div class="page-header">
		<div class="row">
			<div class="col-sm-12">
				<h3 class="page-title">إدارة أعضاء الفرق</h3>
				<!-- <ul class="breadcrumb">
			...
			<div class="col-12 row mt-3">
				...
				<div class="col-12 col-sm-6 col-md-3 mb-2" v-if="isAdmin">
					<label>الفريق</label>
					<select @change="SearchChange" v-model="dataSearch.teamId" class="form-control"
						aria-label="Default select example">
						<option value="0" key="0" selected>-- اختر الفريق --</option>
						<option v-for="item in getTeamsData" :value="item.id" :key="item.id">{{ item.name }}
						</option>
					</select>
				</div>
				...
				<div class="card-body">
					<div class="table-responsive">
						<table class="datatable table table-hover table-center mb-0">
							...
							<tbody>
								<tr v-for="(item, index) in getTeamMembersData">
		...
		<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">إضافة عضو فريق جديد</h5>
				...
				<div class="modal-body">
					<div>
						<div class="row form-row">
							...
							<div class="col-12 col-sm-6" v-if="isAdmin">
								<div class="form-group">
									...
									<select v-model="data.teamId" class="form-control"
										aria-label="Default select example">
										<option value="0" key="0" selected>-- اختر الفريق --</option>
										<option v-for="item in getTeamsData" :value="item.id" :key="item.id">{{
											item.name }}</option>
									</select>
								</div>
							</div>
							...
							<div class="col-12 col-sm-6">
								<div class="form-group">
									<label>معلومات مفصلة عن العنوان</label>
									<textarea v-model="data.address" type="text" class="form-control"></textarea>
								</div>
							</div>
							<div class="col-12 col-sm-12">
								<div class="form-group">
									<label>معلومات أخرى</label>
									<textarea v-model="data.moreInfo" type="text" class="form-control"></textarea>
								</div>
							</div>
							...
							<div class="col-12 col-sm-6">
								<div class="form-group">
									...
									<input v-model="data.confirmPassword" type="password" class="form-control">
								</div>
							</div>
							
							<div class="col-12 col-sm-6">
								<div class="form-group">
									<label>صلاحيات المنسق</label>
								</div>
							</div>
	...
	<div class="modal fade" id="edit_modal" aria-hidden="true" role="dialog">
		<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">تعديل عضو الفريق</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
